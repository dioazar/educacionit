<?php 
	...
	function grabarCookiesLogin($user)
	{
		setcookie('login', 'ok', time()+31556926,'/' );
		setcookie('username', $user, time()+31556926,'/' );
	}
			...
			if($usuario->execute() && $usuario->rowCount() > 0)
			{
				...
				if($usuario['Estado'] != 1)
				{
					...
					if(password_verify( $pass, $passDB ))
					{
						session_start();
						$_SESSION['login'] = 'ok';
						grabarCookiesLogin($usuario['Nombre']);
						$rta = "0x028";
					}else{
						$rta = "0x029";
					}
	...
	function logout()
	{
		...
		session_destroy();
		destroyCookies();
		header('location:index.php?page=ingreso&rta=0x031');
	}
	...
	function getAllProductsPaginados($pagina, $limite)
	{
		global $conn;
		...
		$query = $conn->prepare("SELECT p.idProducto,
								...
								JOIN categorias c ON p.Categoria = c.idCategoria
								LIMIT :inicio, :limite");
		$query->bindParam(":inicio", $inicial, PDO::PARAM_INT);
	...
	function getPaginasProductos($limite)
	{
		global $conn;
		$query = $conn->prepare("SELECT COUNT(*) FROM productos");
		$query->execute();
		$cantProductos = $query->fetchColumn();

		$cantPaginas = ceil($cantProductos/$limite);
		return $cantPaginas;
	}
	...
	function getMarca($id)
	{
		...
		$query = $conn->prepare("SELECT * FROM marcas WHERE idMarca = :id");
		$query->bindParam(':id', $id, PDO::PARAM_INT);
	...
	function updateMarca($id, $marca)
	{
		...
		$query = $conn->prepare("UPDATE marcas SET Nombre = :marca WHERE idMarca = :id");
		$query->bindParam(":marca", $marca, PDO::PARAM_STR);
		$query->bindParam(":id", $id, PDO::PARAM_INT);
		
		if($query->execute())
		{
			header('location:index.php?page=marcas&rta=0x033');
		}else{
			header('location:index.php?page=marcas&rta=0x034');
		}
	}
	...
	function createMarca($marca)
	{
		...
		$query = $conn->prepare("INSERT INTO marcas(Nombre) VALUES(:marca)");
		$query->bindParam(":marca", $marca, PDO::PARAM_STR);
		if($query->execute())
		{
			header('location:index.php?page=marcas&rta=0x033');
	...
	function deleteMarca($id)
	{
		...
		$query = $conn->prepare("DELETE FROM marcas WHERE idMarca = :id");
		$query->bindParam(":id", $id, PDO::PARAM_INT);
		if($query->execute())
		{
			header('location:index.php?page=marcas&rta=0x033');
